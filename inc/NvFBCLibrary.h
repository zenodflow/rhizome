...
public:
    ...
    NVFBCRESULT getStatus(NvFBCStatusEx *status, int adapter = 0)
    {
    ...
    void *create(DWORD type, DWORD *maxWidth, DWORD *maxHeight, int adapter = 0, void *devicePtr = NULL)
    {
        ...
        // Relying on the setTargetAdapter in getStatus to make sure
        // the proper target adaptor is not set.  If getStatus was not
        // called earlier than the NVFBC_TARGET_ADAPTER environment
        // would need to be set before calling create.
